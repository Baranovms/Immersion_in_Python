...
class EmployeeName:
    ...
    def __set__(self, instance, value: str):
        if not all([all(list(map(lambda x: x.isalpha(), name))) for name in value.split()]):
            raise ValueError(f'Имя не может состоять только из букв: {value}')
        if not all(map(lambda x: x.istitle().value.split())):
            raise ValueError(f'Имя должно начинаться с большой буквы: {value}')
...
class Employee:
    ...
    def __int__(self, name: str, lvl_access: int, employee_id: str):
        self.name = name
...
me = nike.login('Изот Эдгарович Никифоров', '531739')
nike.hiring(me, 'Андрей Кривой', '345345', 5)
